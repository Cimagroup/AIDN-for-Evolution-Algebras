...
def generator(input_dim=2,activation_function='sigmoid',bias=False):   
    ...
    #x=Dense(10*input_dim+2, use_bias=bias,activation=activation_function)(x)
    #x=Dense(5000, use_bias=bias,activation=activation_function)(x)
    #x=Dense(2000, use_bias=bias,activation=activation_function)(inputs)
    #x=Dense(2000, use_bias=bias,activation=activation_function)(x)
    x=Dense(1000, use_bias=bias,activation=activation_function)(inputs)
...
def group_rep_loss(input_dim=1,d=2):
    ...
    def loss(y_true,y_pred):
        ...
        i3 = int(d^2*(d-1))
        i4 = d**2
        Ar1 = tf.slice(y_pred,[1],[i1])
        Ar2 = tf.slice(y_pred,[i1],[i2])
        Ar3 = tf.slice(y_pred,[i3],[i4])
        A1=K.mean(math_ops.square(Ar1), axis=0)
        A2=K.mean(math_ops.square(Ar2), axis=0)
        A3=K.max(K.abs(Ar3))
        #A3=math_ops.Sum(Ar3)# axis=0)
        A=A1+A2+A3
        return K.sum(math_ops.square(y_pred),axis=0)
