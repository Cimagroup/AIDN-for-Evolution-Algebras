...
def r1(gs,x):
    ...
    r1_list = [[tf.math.add(
                    tf.linalg.matmul(gs[k,i](x),gs[i,j](x))[0],
                    tf.linalg.matmul(gs[k,j](x),gs[j,i](x))[0]) 
               for (i,j) in indexes if i!=j] 
               for k in range(n)]
...
def r2(gs,x):
    ...
    gs = np.array(gs).reshape((n,n))
    indexes = [(i,j) for i in range(n) for j in range(n)]
    r2_list = [tf.math.subtract(
        tf.multiply(tf.linalg.matmul(gs[i,j](x),gs[j,j](x))[0],2.0),
        tf.math.reduce_sum([tf.linalg.matmul(gs[j,j](x),gs[k,j](x))[0] for k in range(n)])
                ) 
               for (i,j) in indexes]
...
def r3(gs,x): # non-trivial solution
    ...
    n= int(np.sqrt(l))
    gs = np.array(gs).reshape((n,n))
    r3_list = [tf.math.reduce_sum([1/tf.math.reduce_sum([tf.abs(gs[i,j](x)) 
                                                         for i in range(n)])]) 
               for j in range(n)]
    return r3_list
